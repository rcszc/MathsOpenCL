...
bool __mocl_matrixrgb_nullptr(mocl_MatrixImgRGB in) {
...
mocl_MatrixImgRGB MOCL_AddBlend(mocl_MatrixImgRGB matrixA, mocl_fp32 blendA,mocl_MatrixImgRGB matrixB, mocl_fp32 blendB) {
	...
	if (!__mocl_matrixrgb_nullptr(matrixA) && !__mocl_matrixrgb_nullptr(matrixB)) {
		...
		if ((matrixA.mat_width == matrixB.mat_width) && (matrixA.mat_height == matrixB.mat_height)) {
			mocl_MatrixImgRGB returnMat = MOCL_TOOL_NEWMATRIXIMG(matrixA.mat_width, matrixA.mat_height);

			for (mocl_size i = 0; i < returnMat.mat_width; i++) {
				for (mocl_size j = 0; j < returnMat.mat_height; j++) {
					// AddBlend = A * ba + B * bb
					returnMat.dataR[i][j] = matrixA.dataR[i][j] * blendA + matrixB.dataR[i][j] * blendB;
					returnMat.dataG[i][j] = matrixA.dataG[i][j] * blendA + matrixB.dataR[i][j] * blendB;
					returnMat.dataB[i][j] = matrixA.dataB[i][j] * blendA + matrixB.dataR[i][j] * blendB;
				}
			}
		}
		else
		...
		ClogOut << ClogOut.SET(CLOG_WARRING) << "MOPENCL Tool: AddBlend on.nullptr" << CRLOGEND;
