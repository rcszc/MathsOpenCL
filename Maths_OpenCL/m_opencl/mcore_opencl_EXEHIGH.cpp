...
// GPU Mem.io.speed >= 256000 Mb/s
// computing model [Beta], class MHIGH_OPENCL
...
void MHIGH_OPENCL::HMCL_SetWorkGroupSize(mocl_size x, mocl_size y) {
	if ((x > 0) && (y > 0)) {
		...
		CONLOG(CLF_WRITELOG_ERROR, "set work group n > 0", LEVEL_MAX);
}
...
mocl_bit MHIGH_OPENCL::HMCL_Matrix2Dbeta(mocl_Matrix3D inMatrix) {
	...
	if (inMatrix.mat_xlength == _MemObj_in) {
		...
		if (errorcode != CL_SUCCESS) {
			// Execution failed.
			CONLOG(CLF_WRITELOG_ERROR, VATEXT("queueing kernel for execution. code:", errorcode), LEVEL_MAX);
			MOCL_FUNC_CLEARPK(_MOCL_EXDAT);
			returnstate = false;
		}
	}
	else {
		CONLOG(CLF_WRITELOG_ERROR, "inMatrix.mat_xlength != set_in_matrix", LEVEL_MAX);
		returnstate = false;
	}
	return returnstate;
}
...
mocl_Matrix3D MHIGH_OPENCL::HMCL_ReadOutMatrix2D() {
	...
	if (errorcode != CL_SUCCESS) {
		CONLOG(CLF_WRITELOG_ERROR, VATEXT("reading result buffer. code:", errorcode), LEVEL_MAX);
